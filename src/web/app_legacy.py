...
# Page config
st.set_page_config(
    page_title="Lead Crawler", page_icon="🔍", layout="wide", initial_sidebar_state="expanded"
)
...
if page == "🏠 Startseite":
                ...
                try:
                    ...
                    if not coords:
                        ...
                        if use_llm:
                            ...
                            results = run_spider_radius(center_plz=plz_input, radius_km=radius_km)
    ...
    if st.session_state.search_results:
        ...
        if filtered_results:
            ...
            if selected_company:
                company = next(
                    (r for r in filtered_results if r.get("name") == selected_company), None
                )
                if company:
                    with st.container():
                        st.markdown(
        ...
        if any("score_total" in r for r in results):
            ...
            if scores:
                score_df = pd.DataFrame(
                    [{"Grade": k, "Anzahl": v} for k, v in sorted(scores.items())]
                )
