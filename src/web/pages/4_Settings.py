...
with save_col1:
    if st.button("💾 Einstellungen speichern"):
        try:
            # .env Datei schreiben
            env_path = Path(".env")
            env_content = f"""# Lead Crawler Configuration
...
OLLAMA_MAX_RETRIES={retry_attempts}
...
# PLZ Settings
PLZ_DB_PATH={plz_db_path}
PLZ_DEFAULT_RADIUS={default_radius}

# Crawler Settings
CRAWLER_TIMEOUT={request_timeout}
CRAWLER_USER_AGENT={user_agent}
"""
            env_path.write_text(env_content)
            st.success("✅ Einstellungen gespeichert! Bitte App neu starten.")
        except Exception as e:
...
with save_col2:
    if st.button("🔄 Auf Standard zurücksetzen"):
        try:
            ...
            if env_path.exists():
                env_path.unlink()
            st.success("✅ Einstellungen zurückgesetzt! Bitte App neu starten.")
        except Exception as e:
            st.error(f"❌ Fehler: {str(e)}")
